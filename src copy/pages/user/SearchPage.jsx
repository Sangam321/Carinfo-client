...
const SearchPage = () => {
  ...
  const { data, isLoading } = useGetSearchCarQuery({
    searchQuery: query,
    categories: selectedCategories,
    sortByPrice,
  });

  const isEmpty = !isLoading && data?.cars.length === 0;
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <div className="flex-1">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, idx) => <CarSkeleton key={idx} />)
          ) : isEmpty ? (
            <CarNotFound />
          ) : (
            data?.cars?.map((car) => <SearchResult key={car._id} car={car} />)
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-32 dark:bg-gray-900 p-6">
      ...
      <Link to="/" className="italic">
        <Button variant="link">Browse All Cars</Button>
      </Link>
    </div>
  );
};
